...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhosts:
            if ghost:
                distribution = livingGhostPositionDistributions[count]
                vals = list(distribution.values())
                keys = list(distribution.keys())
                beliefPositions.append(keys[vals.index(max(vals))])
                count = count + 1
        minDist = float('inf')
        if beliefPositions:
            targetGhostPosn = None
            for position in beliefPositions:
                distance = self.distancer.getDistance(pacmanPosition, position)
                if distance < minDist:
                    targetGhostPosn = position
                    minDist = distance
            newMin = float('inf')
            chosenAction = None
            for action in legal:
                successorPosition = Actions.getSuccessor(pacmanPosition, action)
                bestDist = self.distancer.getDistance(targetGhostPosn, successorPosition)
                if bestDist < newMin:
                    chosenAction = action
                    newMin = bestDist
            return chosenAction
